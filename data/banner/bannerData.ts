
export const bannerData = [
    {
        title:'Dominate the Competition with the Computer',
        description:'Unleash ultimate gaming power with the Legion X7. Featuring a top-of-the-line processor, powerful graphics card, and a high-refresh-rate display.',
        images:['/images/banner/computer.jpg'],
        button:'Shop Gaming Laptops',
        discountText:'Buy now Get 10% off',
        link: `shop?category=Computers`
    },
    {
        title:'Introducing the All-New router',
        description:'Experience unparalleled performance with our most advanced yet. Blazing-fast processor, stunning camera system, and a revolutionary display.',
        images:['/images/banner/router.jpg'],
        button:'Order Now',
        discountText:'Buy now Get 5% off',
        link:`shop?category=Smartphones`
    },
    {
        title:'High-resolution noise-canceling Printer and Scanner.',
        description:'Immerse Yourself in Pure print with the SonicPro Max',
        images:['/images/products/imprimante2.jpg'],
        button:'Shop Scanners',
        discountText:'Buy now Get 15% off',
        link:'shop?category=Headphones'
    },
]